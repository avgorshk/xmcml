...
void ComputeIntersectionVirtualPlaceInGlassAndWrite(int isFindIntersection, OutputInfo* output,
	Area* area, PhotonState* photon)
{
	double3 direction, virtualPlacePosition;
	int index;
	double z1, z2, sizeCellZ;
	double zVirtualPlace, tempStep;

	sizeCellZ = area->length.z / area->partitionNumber.z;

	direction.x = photon->direction.x;
	direction.y = photon->direction.y;
	direction.z = photon->direction.z;
	
	z1 = photon->position.z;

	if(isFindIntersection)
	{
		z2 = z1 + photon->step * direction.z;
	}
	else
	{
		if(direction.z > 0.0)
		{
			z2 = area->corner.z + area->length.z;
		}
		else
		{
			z2 = area->corner.z;
	...
	zVirtualPlace = sizeCellZ * floor(z1 / sizeCellZ) + sizeCellZ / 2.0;

	if(zVirtualPlace < z1)
	{
		zVirtualPlace += sizeCellZ; 
	}

	while(zVirtualPlace < z2)
	{
		tempStep = (zVirtualPlace - z1) / direction.z;

		virtualPlacePosition.x = photon->position.x + tempStep * direction.x;
		virtualPlacePosition.y = photon->position.y + tempStep * direction.y;
		virtualPlacePosition.z = zVirtualPlace;

		index = GetAreaIndex(virtualPlacePosition, area);

		if(index != -1)
		{
			output->scatteringMap[index] += photon->weight;
		}
...
void ComputeIntersectionVirtualPlaceInTissueAndWrite(OutputInfo* output, Area* area,
	PhotonState* photon)
{
	double3 direction, virtualPlacePosition;
    ...
    while (!photon.isDead)
    {
        ...
        if (isPhotonInGlass)
        {
            ...
            if (intersection.isFindIntersection)
            {
                ...
                ComputeIntersectionVirtualPlaceInGlassAndWrite(intersection.isFindIntersection, output, input->area, &photon);
        ...
        else
        {
            ...
            {
                photon.step = intersection.distance - 0.01 * MIN_DISTANCE;
                ComputeIntersectionVirtualPlaceInTissueAndWrite(output, input->area, &photon);
                MovePhotonAndUpdateTrajectory(&photon, input, trajectory);
...
void LaunchPhoton(InputInfo* input, PhotonState* photon, double specularReflectance)
{
	...
	photon->maxDepth = photon->position.z;
...
void MovePhotonAndUpdateTrajectory(PhotonState* photon, InputInfo* input, PhotonTrajectory* trajectory)
{
	...
	if(photon->position.z > photon->maxDepth)
		photon->maxDepth = photon->position.z;
...
int GetAreaXYIndex(double3 photonPosition, Area* area)
{	
    double indexX = area->partitionNumber.x * (photonPosition.x - area->corner.x) / area->length.x;
    double indexY = area->partitionNumber.y * (photonPosition.y - area->corner.y) / area->length.y;
	...
	return (int)indexY * area->partitionNumber.x + (int)indexX;
}
    ...
    IntersectionInfo* intersection, MCG59* randomGenerator, PhotonTrajectory* trajectory)
{
    ...
    if (NextMCG59(randomGenerator) > reflectance) //transmitting
    {
        ...
        {
            int detectorId;
			int indexGridDetector, indexDepthMap;
			double3 posForDepthMap;

			posForDepthMap.x = photon->position.x;
			posForDepthMap.y = photon->position.y;
			posForDepthMap.z = photon->maxDepth;

			indexDepthMap = GetAreaIndex(posForDepthMap, input->area);

			if(indexDepthMap != -1)
				UpdateWeightInDepthMap(output, photon->weight, indexDepthMap);

			indexGridDetector = GetAreaXYIndex(photon->position, input->area);

			if(indexGridDetector != -1)
				UpdateWeightInGridDetector(output, photon->weight, indexGridDetector);
            
			detectorId = GetDetectorId(photon, input);
...
void UpdateWeightInGridDetector(OutputInfo* output, double photonWeight, int indexGridDetector)
{
	#pragma omp atomic
	output->weightInGridDetector[indexGridDetector] += photonWeight;
}

void UpdateWeightInDepthMap(OutputInfo* output, double photonWeight, int indexDepthMap)
{
	#pragma omp atomic
	output->depthMap[indexDepthMap] += photonWeight;
}
