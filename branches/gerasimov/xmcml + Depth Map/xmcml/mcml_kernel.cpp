	...
	PhotonState* photon)
{
	...
	if(z1 > z2)
	{
		...
		direction.x *= -1.0; direction.y *= -1.0;
		direction.z *= -1.0;
	}
    ...
    IntersectionInfo* intersection, MCG59* randomGenerator, PhotonTrajectory* trajectory)
{
    ...
    if (NextMCG59(randomGenerator) > reflectance) //transmitting
    {
		if (intersectionLayerId == 0)
        {
			...
			{
				double3 tmpPosition;
				tmpPosition.x = photon->position.x; tmpPosition.y = photon->position.y;
				tmpPosition.z = photon->maxDepth;

				int indexDepthMap = GetAreaIndex(tmpPosition, input->area);
				if(indexDepthMap != -1)
					{ UpdateWeightInDepthMap(output, photon->weight, indexDepthMap); }

				int indexGridDetector = GetAreaXYIndex(photon->position, input->area);
