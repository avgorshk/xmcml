...
void SendWeightTablesToAll(WeightIntegralTable* weightTables, int numberOfWeightTables, int pid)
{
    for (int i = 0; i < numberOfWeightTables; ++i)
    {
        MPI_Bcast(&(weightTables[i].anisotropy), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(weightTables[i].numberOfElements), 1, MPI_INT, 0, MPI_COMM_WORLD);
        
        if (pid > 0)
        {
            weightTables[i].elements = new double[weightTables[i].numberOfElements];
        }

        MPI_Bcast(weightTables[i].elements, weightTables[i].numberOfElements, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }
}
...
void SendInputToAll(InputInfo* input, int pid)
{
    ...
    MPI_Bcast(&(input->numberOfWeightTables), 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&(input->weightIntegralPrecision), 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD);
    MPI_Bcast(&(input->weightTablePrecision), 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD);
    ...
    SendWeightTablesToAll(input->weightTable, input->numberOfWeightTables, pid);
}
...
void ReceiveOutputFromAll(OutputInfo* output, int pid)
{
	...
	double* scatteringMap = NULL;
	double* depthMap = NULL;
    double* weightInDetector = NULL;
	double* weightInGridDetector = NULL;
    uint64* trajectory = NULL;
    uint64 numberOfPhotonsPerDetector = 0;
    uint64 numberOfPhotonsPerTime = 0;
    double weightPerTime = 0.0;
	double totalRangePerDetector = 0.0;
	double targetRangePerDetector = 0.0;
    ...
    if (pid == 0)
    {
		...
		scatteringMap = new double[output->gridSize];
		depthMap = new double[output->gridSize];
        weightInDetector = new double[output->numberOfDetectors];
		weightInGridDetector = new double[output->gridDetectorSize];
    }

    MPI_Reduce(output->absorption, absorption, output->gridSize, 
        MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(output->scatteringMap, scatteringMap, output->gridSize,
		MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(output->depthMap, depthMap, output->gridSize,
		MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(output->weightInDetector, weightInDetector, output->numberOfDetectors, 
        MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(output->weightInGridDetector, weightInGridDetector, output->gridDetectorSize,
		MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
...
void DoBackup(InputInfo* input, OutputInfo* output, MCG59* randomGenerator, int numThreads, uint64 numberOfPhotonsCount)
{
    ...
    else
    {
		...
		memset(output->scatteringMap, 0, output->gridSize * sizeof(double));
		memset(output->depthMap, 0, output->gridSize * sizeof(double));
		memset(output->weightInDetector, 0, output->numberOfDetectors * sizeof(double));
		memset(output->weightInGridDetector, 0, output->gridDetectorSize * sizeof(double));
        for (int i = 0; i < output->numberOfDetectors; ++i)
        {
