...
int ReadSectionDepthMap(FILE* file, OutputInfo* output)
{
    unsigned long long int reading_items;
    int section;
    unsigned int section_length;
    ...
    reading_items = fread(&section, sizeof(int), 1, file);
    if (reading_items < 1 || section != MCML_SECTION_DEPTH_MAP)
        return -1;
    ...
    reading_items = fread(output->depthMap, sizeof(double), gridSize, file);
    if (reading_items < gridSize)
        return -1;
...
int ReadSectionGridDetector(FILE* file, OutputInfo* output)
{
    unsigned long long int reading_items;
    int section;
    unsigned int section_length;
    ...
    reading_items = fread(&section, sizeof(int), 1, file);
    if (reading_items < 1 || section != MCML_SECTION_GRID_DETECTOR)
        return -1;
	...
	if (reading_items < 1 || gridDetectorSize != output->gridDetectorSize)
        return -1;

	reading_items = fread(output->weightInGridDetector, sizeof(double), gridDetectorSize, file);
