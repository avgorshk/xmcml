	...
	Area* area, PhotonState* photon)
{
	...
	if(isFindIntersection) z2 = z1 + photon->step * direction.z;
	else
	{
		if(direction.z > 0.0) z2 = area->corner.z + area->length.z;
		else z2 = area->corner.z;
	}
    ...
    IntersectionInfo* intersection, MCG59* randomGenerator, PhotonTrajectory* trajectory)
{
    ...
    if (NextMCG59(randomGenerator) > reflectance) //transmitting
    {
        if (intersectionLayerId == 0)
        {
			...
			int indexGridSize = GetAreaXYIndex(photon->position, input->area);

			if(indexGridSize!=-1)
				UpdateWeightInGridDetector(output, photon->weight, indexGridSize);
...
void UpdateWeightInGridDetector(OutputInfo* output, double photonWeight, int indexGridSize)
{
	#pragma omp atomic
	output->weightInGridDetector[indexGridSize] += photonWeight;
}
