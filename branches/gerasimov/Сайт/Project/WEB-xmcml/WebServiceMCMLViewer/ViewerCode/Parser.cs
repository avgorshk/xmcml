...
namespace mcmlVisualizer
{
    class Parser
    {
        ...
        static public Parser getInstance(string filePath)
        {
            ...
            {
                parser = new Parser(filePath, sections);
            }
            file.Close();
        ...
        private Parser(String filePath, Hashtable sections)
        {
        ...
        private void Init()
        {
            FileStream file = File.Open(this.filePath, FileMode.Open, FileAccess.Read, FileShare.Read);
            BinaryReader reader = new BinaryReader(file);
            uint offset = 0;
        ...
        public double[] GetTrajectoriesAbsorption()
        {
        ...
        public UInt64[] GetDetectorTrajectories(int detectorId)
        {
            ...
            int numberOfDetectors = reader.ReadInt32();
            UInt64[] trajectory = new UInt64[0];
            for (int i = 0; i < numberOfDetectors; ++i)
            {
                ...
                if (i == detectorId)
                {
                    trajectory = new UInt64[numberOfValues];
