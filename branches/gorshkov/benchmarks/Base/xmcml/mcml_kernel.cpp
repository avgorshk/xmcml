...
void MovePhotonAndUpdateTrajectory(PhotonState* photon, InputInfo* input, OutputInfo* output, uint64* trajectory)
{
...
void MovePhotonOnMinDistanceAndUpdateTrajectory(PhotonState* photon, InputInfo* input, OutputInfo* output, uint64* trajectory)
{
    double3 previousPhotonPosition = photon->position;
...
void UpdatePhotonTrajectory(PhotonState* photon, InputInfo* input, OutputInfo* output, uint64* trajectory, 
    double3 previousPhotonPosition)
{
    ...
    while ((plane <= finishPosition.z) && (plane < input->area->corner.z + input->area->length.z))
    {
        ...
        ++(output->commonTrajectory[areaIndex]);
