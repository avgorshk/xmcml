...
static int WriteSectionCommonTrajectories(FILE* file, uint64* commonTrajecotory, int gridSize)
{
    unsigned long long int written_items;
    ...
    written_items = fwrite(commonTrajecotory, sizeof(uint64), gridSize, file);
...
bool WriteOutputToFile(InputInfo* input, OutputInfo* output, char* fileName)
{
    ...
    if (WriteSectionSpecularReflectance(file, output->specularReflectance) != 0)
        return false;
    if (WriteSectionCommonTrajectories(file, output->commonTrajectory, output->gridSize) != 0)
        return false;
