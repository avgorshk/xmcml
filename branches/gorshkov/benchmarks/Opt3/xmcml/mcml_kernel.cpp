    ...
    while (!photon.isDead)
    {
        ...
        if (isPhotonInGlass)
        {
            ...
            if (intersection.isFindIntersection)
            {
                ...
                MovePhotonAndUpdateTrajectory(&photon, input, output, trajectory);
...
void MovePhotonAndUpdateTrajectory(PhotonState* photon, InputInfo* input, OutputInfo* output, 
    PhotonTrjectory* trajectory)
{
    double3 previousPhotonPosition = photon->position;
    MovePhoton(photon);
    UpdatePhotonTrajectory(photon, input, output, trajectory, previousPhotonPosition);
}
...
void MovePhotonOnMinDistanceAndUpdateTrajectory(PhotonState* photon, InputInfo* input, 
    OutputInfo* output, PhotonTrjectory* trajectory)
{
    double3 previousPhotonPosition = photon->position;
...
void CrossBoundary(PhotonState* photon, InputInfo* input, OutputInfo* output, 
    IntersectionInfo* intersection, MCG59* randomGenerator, PhotonTrjectory* trajectory)
{
    double reflectance;
    double transmitCos;
    ...
    if (NextMCG59(randomGenerator) > reflectance) //transmitting
    {
        ...
        else
        {
            ...
            MovePhotonOnMinDistanceAndUpdateTrajectory(photon, input, output, trajectory);
        }
    }
    else //reflection
    {
...
void ComputeDroppedWeightOfPhoton(PhotonState* photon, InputInfo* input, OutputInfo* output, int droppedIndex)
{
	...
	photon->weight -= photon->weight * absorptionCoefficient / (absorptionCoefficient + scatteringCoefficient);
}
    ...
    PhotonTrjectory* trajectory, double3 previousPhotonPosition)
{
    ...
    while ((plane <= finishPosition.z) && (plane < input->area->corner.z + input->area->length.z))
    {
        ...
        trajectory->x[trajectory->size] = areaIndexVector.x;
        trajectory->y[trajectory->size] = areaIndexVector.y;
        trajectory->z[trajectory->size] = areaIndexVector.z;
        ++(trajectory->size);

        int index = areaIndexVector.x*input->area->partitionNumber.y*input->area->partitionNumber.z +
            areaIndexVector.y*input->area->partitionNumber.z + areaIndexVector.z;
...
void UpdateDetectorTrajectory(OutputInfo* output, Area* area, PhotonTrjectory* trajectory, int detectorId)
{
    ...
    {
        index = trajectory->x[i]*ny*nz + trajectory->y[i]*nz + trajectory->z[i];
