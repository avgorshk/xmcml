...
void LaunchGPUWithBackups(InputInfo* input, OutputInfo* output, MCG59* randomGenerator, 
    int numThreads, uint backupPortionSize)
{
    ...
    for (uint64 i = 0; i < numberOfBackupPortions; ++i)
    {
        ...
        LaunchGPU(input, output, randomGenerator);
    ...
    int numThreadsPerProcess, uint backupPortionSize)
{
    ...
    SetGpuDevice(mpi_rank);
    LaunchGPUWithBackups(input, output, randomGenerator, numThreadsPerProcess, backupPortionSize);
