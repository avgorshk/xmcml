...
namespace mcmlVisualizer
{
    class Parser
    {
                ...
                for (int i = 0; i < numberOfDetectors; ++i)
                {
                    ...
                    if (i == detectorId)
                    {
                        ...
                        for (int j = 0; j < timeScaleSize; ++j)
                        {
                            ...
                            int trajectorySize = reader.ReadInt32();
                            reader.ReadBytes(trajectorySize * sizeof(UInt64));
                        }
                        return timeInfo;
                    }
                    for (int j = 0; j < timeScaleSize; ++j)
        ...
        public UInt64[] GetDetectorTimeTrajectory(int detectorId, int timeId)
        {
            BinaryReader reader = new BinaryReader(this.file);
            if (this.sections[(uint?)MCML_SECTION_DETECTOR_TIME_SCALE] != null)
            {
                ...
                for (int i = 0; i < numberOfDetectors; ++i)
                {
                    ...
                    if (i == detectorId)
                    {
                        for (int j = 0; j < timeScaleSize; ++j)
                        {
                            ...
                            {
                                int trajectorySize = reader.ReadInt32();
                                UInt64[] trajectory = new UInt64[trajectorySize];
                                for (int k = 0; k < trajectorySize; ++k)
                                {
                                    trajectory[k] = reader.ReadUInt64();
                                }
                                return trajectory;
                            }
                            else
