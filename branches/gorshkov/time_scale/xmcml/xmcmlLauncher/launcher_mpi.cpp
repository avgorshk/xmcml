...
void ReceiveOutputFromAll(OutputInfo* output, int pid)
{
    ...
    for (int i = 0; i < output->numberOfDetectors; ++i)
    {
        if (pid == 0)
        {
            trajectory = new uint64[output->detectorTrajectory[i].trajectorySize];
            numberOfPhotonsPerDetector = 0;
        }
        
        MPI_Reduce(output->detectorTrajectory[i].trajectory, trajectory, 
            output->detectorTrajectory[i].trajectorySize, 
        ...
        for (int j = 0; j < output->detectorTrajectory[i].timeScaleSize; ++j)
        {
            if (pid == 0)
            {
                ...
                timeTrajectory = new uint64[output->detectorTrajectory[i].timeScale[j].trajectorySize];
            }
            ...
            MPI_Reduce(output->detectorTrajectory[i].timeScale[j].trajectory, timeTrajectory,
                output->detectorTrajectory[i].timeScale[j].trajectorySize,
                MPI_UNSIGNED_LONG_LONG, ullSumOp, 0, MPI_COMM_WORLD);
            ...
            if (pid == 0)
            {
                ...
                output->detectorTrajectory[i].timeScale[j].trajectory = timeTrajectory;
            }
        }
    }
...
void DoBackup(InputInfo* input, OutputInfo* output, MCG59* randomGenerator, int numThreads, uint64 numberOfPhotonsCount)
{
    ...
    if (mpi_rank == 0)
    {
        ...
        sprintf_s(message, "%s", isOk ? "Backup file is written" : "Backup file is NOT written");
        WriteMessageToLog(message);
    }
    else
    {
        ...
        for (int i = 0; i < output->numberOfDetectors; ++i)
        {
            ...
            for (int j = 0; j < output->detectorTrajectory[i].timeScaleSize; ++j)
            {
                ...
                memset(output->detectorTrajectory[i].timeScale[j].trajectory, 0, 
                    output->detectorTrajectory[i].timeScale[j].trajectorySize * sizeof(uint64));
            }
        }
    }
}
