...
static char* NumberToString(uint64 number)
{
    ...
    {
        sprintf_s(result, bufferSize, "%.2f%c", number / 1000000000.0f, 'B');
    }
    else if (number >= 1000000) 
    {
        sprintf_s(result, bufferSize, "%.2f%c", number / 1000000.0f, 'M');
    }
    else if (number >= 1000) 
    {
        sprintf_s(result, bufferSize, "%.2f%c", number / 1000.0f, 'T');
    }
    else 
    {
        sprintf_s(result, bufferSize, "%u", (uint)number);
    }
    ...
    int numberOfDetectors)
{
    ...
    for (int i = 0; i < numberOfDetectors; ++i)
    {
        ...
        {
            section_lenght += 2 * sizeof(double) + sizeof(uint64) + 
                sizeof(int) + detectorTrajectory[i].timeScale[j].trajectorySize * sizeof(uint64);
        }
    }
    ...
    for (int i = 0; i < numberOfDetectors; ++i)
    {
        ...
        for (int j = 0; j < detectorTrajectory[i].timeScaleSize; ++j)
        {
            ...
            written_items = fwrite(&(detectorTrajectory[i].timeScale[j].trajectorySize), 
                sizeof(int), 1, file);
            if (written_items < 1)
                return -1;

            written_items = fwrite(detectorTrajectory[i].timeScale[j].trajectory, 
                sizeof(uint64), detectorTrajectory[i].timeScale[j].trajectorySize, file);
            if (written_items < detectorTrajectory[i].timeScale[j].trajectorySize)
                return -1;
        }
    }
...
bool WriteOutputToFile(InputInfo* input, OutputInfo* output, char* fileName)
{
    ...
    if (WriteSectionArea(file, input->area) != 0)
        return false;
    if (WriteSectionDetectors(file, input->detector, input->numberOfDetectors) != 0)
        return false;
    ...
    int numThreadsPerProcess, int numProcesses)
{
    ...
    sprintf_s(fileName, "xmcml_%s.mcml.bk", buffer);
    delete[] buffer;
