...
if __name__ == '__main__':
    ...
    while i < args.numberOfComputations:
        ...
        i = i + 1
    import subprocess
    subprocess.call(['Convolution.exe', '-i', str(0) + '.mcml.out', '-o', args.outputFileName, '-l', str(args.convolutionSetting), '-w', '0', '-d', '0'])
    i = 1
    #print('D:\Convolution.exe', '-i', str(0) + '.mcml.out', '-o', args.outputFileName, '-l', str(args.convolutionSetting), '-w', '1', '-d', '0')
    while i < args.numberOfComputations:
        print i
        import subprocess
        subprocess.call(['Convolution.exe', '-i', str(i) + '.mcml.out', '-o', args.outputFileName, '-l', str(args.convolutionSetting), '-w', '1', '-d', '0'])
        i = i + 1
