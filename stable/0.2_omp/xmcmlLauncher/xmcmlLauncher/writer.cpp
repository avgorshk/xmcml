...
int WriteOutputToFile(InputInfo* input, OutputInfo* output, char* fileName)
{
    ...
    if (WriteSectionSpecularReflectance(file, output->specularReflectance) == -1)
        return -1;
    if (WriteSectionTrajectories(file, output->absorption, output->absorptionSize) == -1)
        return -1;
    if (WriteSectionDetectorWeights(file, output->weigthInDetector, output->numberOfDetectors) == -1)
        return -1;
