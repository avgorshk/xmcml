...
namespace mcmlVisualizer
{
    class Parser
    {
        ...
        public int GetNumberOfPhotons() 
        {
            ...
            int numberOfPhotons = reader.ReadInt32();
        ...
        public double[] GetDetectorTrajectories(int detectorId)
        {
            BinaryReader reader = new BinaryReader(this.file);
            ...
            for (int i = 0; i < numberOfDetectors; ++i)
            {
                ...
                if (i == detectorId)
                {
                    double[] trajectory = new double[numberOfValues];
                    for (int j = 0; j < numberOfValues; ++j)
                    {
                        trajectory[j] = reader.ReadDouble();
                    }
                    return trajectory;
                }
                else
        ...
        public int GetNumberOfPhotonsInDetector(int detectorId)
        {
            BinaryReader reader = new BinaryReader(this.file);
            uint offset = (uint)(this.sections[(uint?)MCML_SECTION_DETECTOR_TRAJECTORIES]);
            reader.BaseStream.Seek(offset, SeekOrigin.Begin);
