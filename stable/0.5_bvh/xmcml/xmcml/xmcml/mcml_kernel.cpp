...
void ComputePhoton(double specularReflectance, InputInfo* input, OutputInfo* output, 
    MCG59* randomGenerator, double* trajectory)
{
    PhotonState photon;
    ...
    memset(trajectory, 0, trajectorySize * sizeof(double));
...
{	
    int indexX = (int)(area->partitionNumber.x * (photonPosition.x - area->corner.x) / area->length.x);
    int indexY = (int)(area->partitionNumber.y * (photonPosition.y - area->corner.y) / area->length.y);
    int indexZ = (int)(area->partitionNumber.z * (photonPosition.z - area->corner.z) / area->length.z);
...
void CrossBoundary(PhotonState* photon, InputInfo* input, OutputInfo* output, 
    IntersectionInfo* intersection, MCG59* randomGenerator, double* trajectory)
{
    double reflectance;
    double transmitCos;
...
void UpdatePhotonTrajectory(PhotonState* photon, InputInfo* input, double* trajectory, 
    double3 previousPhotonPosition)
{
    ...
    while ((plane <= finishPosition.z) && (plane < input->area->corner.z + input->area->length.z))
    {
        ...
        trajectory[areaIndex] += 1.0;
        
        plane += step;
    }
}

void UpdateDetectorTrajectory(OutputInfo* output, double* trajectory, int detectorId)
{
    #pragma omp critical
    {
        double* detectorTrajectory = output->detectorTrajectory[detectorId].trajectory;
