...
int main(int argc, char* argv[])
{
    ...
    double3 center_left = center;
    center_left.x = (-length/2 - radius)/2;
    GeneratePlane(center_left, length/2 - radius, length, &(surface[3]));

    double3 center_right = center;
    center_right.x = (length/2 + radius)/2;
    GeneratePlane(center_right, length/2 - radius, length, &(surface[4]));

    GenerateBottomHalfCylinderXZ(center, radius, length, 10, &(surface[5]));
    GenerateTopHalfCylinderXZ(center, radius, length, 10, &(surface[6]));
    ...
    int result;
    result = WriteOutputToFile(surface, 9, "../simple_tube_15mm.surface");
    printf("Writing data to file...%s\n", (result == -1) ? "FALSE" : "OK");
