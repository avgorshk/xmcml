...
bool IntersectAABB(const AABB& Box, const double3& origin, const double3& direction, double& t_near, double& t_far)
{
	...
	for ( int i = 0; i < 3; i++ ) {
		if ( abs(direction.cell[i]) > EPSILON ) {
            t = 1.0 / direction.cell[i];
			t1 = ( Box.Ver1.cell[i] - origin.cell[i] ) * t;
			t2 = ( Box.Ver2.cell[i] - origin.cell[i] ) * t;
